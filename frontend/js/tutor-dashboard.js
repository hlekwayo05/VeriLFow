...
function tutorPositionLabel() {
  const app = tutorApplication || window.tutorApplication;
  return app?.position_type === 'demonstrator' ? 'Demonstrator' : 'Tutor';
}

function applyTutorPositionBranding() {
  const label = tutorPositionLabel();
  document.title = `VeriFlow - ${label} Dashboard`;
  const section = document.querySelector('.nav-section-label');
  if (section) section.textContent = label;
  const hubNav = document.querySelector('.td-hub-grid[aria-label]');
  if (hubNav) hubNav.setAttribute('aria-label', `${label} sections`);
...
function downloadRegister() {
  ...
  const roleLabel = tutorPositionLabel();
  const tutorFullName = app
    ? `${app.first_names || ''} ${app.surname || ''}`.trim()
    : `${st.user?.firstNames || ''} ${st.user?.surname || ''}`.trim() || roleLabel;
...
<div class="hdr"><div><h1>Attendance Register</h1><p>${modCode}${currentModuleName ? ' - ' + currentModuleName : ''} · ${dateLabel}</p><p style="margin-top:3px;font-size:11px;color:#999;font-family:'DM Mono',monospace">${meta}</p></div><div><div class="brand">VERIFLOW</div><p style="font-size:11px;color:#999;font-family:'DM Mono',monospace">${roleLabel} Management System</p></div></div>
<div class="meta"><div class="mb"><div class="ml">${roleLabel}</div><div class="mv">${tutorFullName}</div></div><div class="mb"><div class="ml">Student No.</div><div class="mv" style="font-family:'DM Mono',monospace">${studentNo}</div></div><div class="mb"><div class="ml">Module</div><div class="mv">${modCode}</div></div><div class="mb"><div class="ml">Present</div><div class="mv" style="color:#1a7a52">${present} / ${enrolled}</div></div><div class="mb"><div class="ml">Rate</div><div class="mv" style="color:#c8a84b">${pct}%</div></div></div>
...
<div class="sig"><div><div class="sl">${roleLabel} Signature</div><div class="ss">${tutorFullName} · ${studentNo}</div></div><div><div class="sl">Lecturer Verification</div><div class="ss">${lecturerName}</div></div></div>
...
function renderTimesheetView(ts) {
  ...
  const hasStaffNumber = ts.hasStaffNumber !== false && !!(
    tutorCurrentUser?.staff_number || tutorApplication?.staff_number || ts.hasStaffNumber
  );
  ...
  const submitLabel = isReturned
    ...
    : (ts.claim ? 'Submitted' : 'Submit to Lecturer');
  ...
  const staffBanner = !hasStaffNumber && !ts.claim
    ? `<div class="ts-info-bar ts-banner-warn">
      <span>You cannot submit claims until HR assigns your staff number. Forms and acceptance are on Profile.</span>
    </div>`
    : '';
  ...
  const unclaimedBanner = unclaimedCount && ts.claim
    ? (ts.claim.status === 'approved'
      ...
      : (ts.claim.status === 'pending_lecturer'
        ? `<div class="ts-info-bar">You have ${unclaimedCount} new completed session(s) not on this claim. They will be included when your lecturer approves (or ask SEO to return the claim so you can resubmit).</div>`
        : (isReturned
          ? `<div class="ts-info-bar">Include all confirmed sessions for this month when you resubmit.</div>`
          : '')))
    : '';
  ...
  wrap.innerHTML = `
    ...
    <div class="submit-bar">
      ...
      <button type="button" class="submit-btn" onclick="submitTimesheet()" ${canSubmit ? '' : 'disabled'}>${submitLabel}</button>
    </div>
    ${pastClaims.length ? `
...
async function submitTimesheet() {
  if (!currentTimesheet?.canSubmit) return;
  if (currentTimesheet.hasStaffNumber === false) {
    showToast('Wait until HR assigns your staff number before submitting claims');
    return;
  }
  const sessionIds = currentTimesheet.sessions.map((s) => s.id);
  if (!sessionIds.length) {
    showToast('No confirmed completed sessions to submit for this month');
    return;
  }
  ...
  try {
    ...
    if (btn) {
      btn.disabled = false;
      btn.textContent = isResubmit ? 'Resubmit to Lecturer' : 'Submit to Lecturer';
    }
  }
}
...
function syncBottomNav(pageId) {
  // Hour Log, Calendar, Support stay hub-only destinations (no bottom-nav tab).
  const map = {
    dashboard: 'dashboard',
    sessions: 'sessions',
    hourlog: null,
...
function renderProfile() {
  ...
  const roleLabel = tutorPositionLabel();
  const roleLine = [titleLabel, roleLabel, moduleLabel, qualLabel ? `${qualLabel} student` : null]
    .filter(Boolean)
    .join(' · ') || roleLabel;
...
async function saveProfileUpdate() {
  ...
  if (studentNumber && !/^\d{8,12}$/.test(studentNumber)) {
    showToast('Student number must be 8–12 digits');
    return;
  }
  if (cellPhone && typeof VF.isValidSaCellPhone === 'function' && !VF.isValidSaCellPhone(cellPhone)) {
    showToast('Enter a valid SA cell number (e.g. 082 456 7890)');
    return;
  }
...
function renderTutorTickets(tickets) {
  ...
  wrap.innerHTML = list.map((t) => {
    ...
    return `<article class="st-ticket-card st-ticket-card--clickable" role="button" tabindex="0" onclick="openTutorTicketDetail(${t.id})" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openTutorTicketDetail(${t.id});}">
      <div class="st-ticket-top">
        <div class="st-ticket-title"><span class="st-ticket-id">#${t.id}</span> · ${escapeHtml(t.subject || '-')}</div>
        <span class="st-badge ${status.cls}">${status.label}</span>
      </div>
      <div class="st-ticket-meta">${metaParts.join(' · ')}</div>
      <div class="st-ticket-details">${escapeHtml(t.details || '')}</div>
    </article>`;
  }).join('');
}
...
function renderTutorSupportThread(replies, containerEl) {
  if (!containerEl) return;
  if (!replies || !replies.length) {
    containerEl.innerHTML = '<div class="st-thread-empty">No replies yet. SEO will respond here.</div>';
    return;
  }
  containerEl.innerHTML = replies.map((r) => {
    const mine = r.author_role === 'tutor';
    const meta = `${escapeHtml(r.author_name || r.author_role || '-')} · ${supportFormatDate(r.created_at)}`;
    return `<div class="st-thread-msg${mine ? ' mine' : ''}">
      <div class="st-thread-meta">${meta}</div>
      <div class="st-thread-bubble">${escapeHtml(r.message)}</div>
    </div>`;
  }).join('');
}

let activeTutorTicketId = null;
let activeTutorTicketCache = null;

async function openTutorTicketDetail(ticketId) {
  const overlay = document.getElementById('ticket-detail-overlay');
  if (!overlay) return;
  try {
    const ticket = await VF.apiFetch(`/support/tickets/${ticketId}`);
    activeTutorTicketId = ticketId;
    activeTutorTicketCache = ticket;

    const status = ticketStatusMeta(ticket.status);
    setText('#ticket-detail-title', `Ticket #${ticket.id}`);
    setText('#ticket-detail-subject', ticket.subject || '-');
    const statusEl = document.getElementById('ticket-detail-status');
    if (statusEl) {
      statusEl.className = `st-badge ${status.cls}`;
      statusEl.textContent = status.label;
    }
    const detailsEl = document.getElementById('ticket-detail-details');
    if (detailsEl) detailsEl.textContent = ticket.details || '';
    renderTutorSupportThread(ticket.replies, document.getElementById('ticket-detail-thread'));

    const replyWrap = document.getElementById('ticket-detail-reply-wrap');
    const msgEl = document.getElementById('ticket-detail-message');
...
function closeTutorTicketDetail(e) {
  ...
  activeTutorTicketId = null;
  activeTutorTicketCache = null;
}

async function sendTutorTicketReply() {
  if (!activeTutorTicketId) return;
  const message = document.getElementById('ticket-detail-message')?.value.trim();
  if (!message) {
    document.getElementById('ticket-detail-message')?.focus();
    return;
  }
  const btn = document.getElementById('ticket-detail-send-btn');
  if (btn) {
    btn.disabled = true;
    btn.textContent = 'Sending…';
  }
  try {
    const reply = await VF.apiFetch(`/support/tickets/${activeTutorTicketId}/reply`, {
      method: 'POST',
      body: { message },
    });
    if (!activeTutorTicketCache) activeTutorTicketCache = { replies: [] };
    if (!activeTutorTicketCache.replies) activeTutorTicketCache.replies = [];
    activeTutorTicketCache.replies.push(reply);
    renderTutorSupportThread(
      activeTutorTicketCache.replies,
      document.getElementById('ticket-detail-thread')
    );
    document.getElementById('ticket-detail-message').value = '';
...
window.openTutorTicketDetail = openTutorTicketDetail;
window.closeTutorTicketDetail = closeTutorTicketDetail;
window.sendTutorTicketReply = sendTutorTicketReply;
